...
import { sendDM, noro } from "./functions.js";
...
export default async function (debug = false, config, is_replit = (process.env.REPL_ID && process.env.REPL_SLUG && process.env.REPL_OWNER)) {
  ...
  const spinner = createSpinner('Run a project made by Noro', { interval: 50 }).start({ "color": "blue" })
  ...
  bot.on("ready", async () => {
    ...
    bot.editStatus(`online`, [{ name: await settings.get("status_bot") || "Bot By Noro", type: await settings.get("status_type") || 0 }]);
    if (!ready_first) {
